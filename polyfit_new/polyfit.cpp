...
template<typename T>
std::vector<T> MathAlgo::polyfit(const std::vector<T>& oX, const std::vector<T>& oY, int nDegree)
{
	if (oX.size() != oY.size())
...
*/
template<typename T>
std::vector<T> MathAlgo::polyval(const std::vector<T>& oCoeff, const std::vector<T>& oX)
{
	size_t nCount = oX.size();
